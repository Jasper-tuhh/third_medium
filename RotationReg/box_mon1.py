...
def run_case(Nx, Ny, fe_degree, target_displacement=-0.50,
    ...
    "box_mon" domain: a single monolithic rectangular block with an
    third-medium band.
    ...
    # ---- Mesh (box_mon: monolithic rectangular block) ----
    p0 = [0.0, 0.0]
    p1 = [2.0, 0.5]
    ...
    # ---- Material tagging (box_mon: embedded third-medium band) ----
    tol = 1e-8
    marker = np.zeros(num_local_cells, dtype=np.int32)
    cells_medium = locate_entities(
        mesh0, tdim,
        lambda x: (x[0] >= 0.1 - tol) & (x[0] <= p1[0] - 0.1 + tol) &
                  (x[1] >= 0.1 - tol) & (x[1] <= 0.4 + tol),
    )
    marker[cells_medium] = 2

    all_potential_body = locate_entities(
        mesh0, tdim,
        lambda x: (x[0] <= 0.1 + tol) | (x[0] >= p1[0] - 0.1 - tol) |
                  (x[1] <= 0.1 + tol) | (x[1] >= 0.4 - tol),
    )
    ...
    # ---- BCs (box_mon: pinned corners + top-midpoint displacement) ----
    left_corner = lambda x: np.isclose(x[0], p0[0]) & np.isclose(x[1], p0[1])
    right_corner = lambda x: np.isclose(x[0], p1[0]) & np.isclose(x[1], p0[1])
    top_midpoint = lambda x: np.isclose(x[0], 0.5 * (p0[0] + p1[0])) & np.isclose(x[1], p1[1])

    left_facet = locate_entities_boundary(mesh0, 0, left_corner)
    right_facet = locate_entities_boundary(mesh0, 0, right_corner)
    top_vertex = locate_entities_boundary(mesh0, 0, top_midpoint)

    # Fully pin the left corner (both displacement components)
    ul = np.array((0,) * mesh0.geometry.dim, dtype=default_scalar_type)
    bcl = dirichletbc(ul, locate_dofs_topological(Vu, 0, left_facet), Vu)

    # Pin the vertical displacement at the right corner (roller support)
    ur = default_scalar_type(0)
    bcr = dirichletbc(ur, locate_dofs_topological(Vu.sub(1), 0, right_facet), Vu.sub(1))
    ...
    Pi_total = Pi_body + Pi_m + Pi_tan_R
...
if __name__ == "__main__":
    ...
    ax.set_ylabel(r"$|Q_h-Q_{\mathrm{ref}}|$")
    ax.set_title("Error vs. computational time")

    ax.grid(True, which="both", linestyle="--", linewidth=0.5, alpha=0.6)
    ax.legend()
    fig.tight_layout()
    fig.savefig(f"{dir_name}/Error_vs_Time.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig(f"{dir_name}/Error_vs_DOF.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
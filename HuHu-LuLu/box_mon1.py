...
def run_case(Nx, Ny, fe_degree, target_displacement=-0.50,
    ...
    # Regularization Laplace Term
    Lu = div(grad(u))

    k_HuHu = default_scalar_type(1.0e-6)
    k_LuLu = default_scalar_type(1.0e-6)

    Pi_HuHu_LuLu = (
        k_HuHu * inner(Hu, Hu) - (k_LuLu / tr(I)) * inner(Lu, Lu)
    ) * dx(2)

    Pi_total = Pi_body + Pi_m + Pi_HuHu_LuLu

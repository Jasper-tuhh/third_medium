...
def run_case(lc, fe_degree, target_load=target_load, Nsteps=Nsteps, write_outputs=True,
    ...
    # HuHu-LuLu regularization on the third medium (dx(3)):
    # Psi = W + Hu:Hu - (1/Tr(I)) * Lu.Lu
    Hu = grad(grad(u))          # rank-3 Hessian of u (nonzero: degree_u >= 2)
    Lu = div(grad(u))           # vector Laplacian of u
    ...
    # HuHu-LuLu is a function of u, contributes no
    # stress -- only the physical third-medium energy does.
    ...
    if plot_deformation and comm.rank == 0:
        try:
            ...
            fig1.tight_layout()
            fig1.savefig(f"{file_name}/displacement_deformed_hertz_lc_{lc:g}_p{fe_degree}.png", dpi=200)
            plt.close(fig1)
        except Exception as e:
            print(f"Post-processing plot failed for lc={lc:g} p={fe_degree}: {e}")
...
if __name__ == "__main__":
    ...
    if comm.rank == 0:
        ...
        try:
            ...
            # Plot 1: Potential Energy vs DOFs
            plt.figure()
            ...
            # Plot 2: Potential Energy vs Mesh Size lc
            plt.figure()
            for i, degree in enumerate(fe_degrees):
                filtered = [r for r in results if r["fe_degree"] == degree]
                h_vals = [r["lc"] for r in filtered]
                energy = [r["energy_bodies"] for r in filtered]

                sorted_pairs = sorted(zip(h_vals, energy))
                h_sorted, energy_sorted = zip(*sorted_pairs)

                plt.plot(h_sorted, energy_sorted, marker=markers[i % len(markers)], label=f"p={degree}", linestyle='-')
            ...
            plt.legend()
            plt.tight_layout()
            plt.savefig(f"{file_name}/bodies_potential_energy_vs_lc.png", dpi=200)
            plt.close()
